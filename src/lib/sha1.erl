...
-module(sha1).
-export([binstring/1, hexstring/1, binfile/1, hexfile/1, encode_bin/1]).
...
encode_bin(S) ->
	case catch binstring_unsafe(S) of 
...
compute_wt(Ws, 80) -> Ws;
compute_wt(Ws, T) -> 
	Wp = lists:map(fun(X)->lists:nth(1 + T - X, Ws) end, [3,8,14,16]),
	Wt_pre = lists:foldl(fun(X,Y)->X bxor Y end, 0, Wp),
	Wt = rotl(1, Wt_pre),
...
inner_loop(T, {A,B,C,D,E}, Ws) ->
	Temp = add([rotl(5, A), f(T, B,C,D), E, lists:nth(1+T, Ws), k(T)]),
